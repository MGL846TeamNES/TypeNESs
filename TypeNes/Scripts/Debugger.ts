...
export class Debugger {
    ...
    public DrawPalette(canvasID: string) {
        ...
        for (let i = 0; i < 16; i++) {
            const r = this.machine.ppu.palettes[0].RGBColors[i] & 0xff;
            const g = (this.machine.ppu.palettes[0].RGBColors[i] >> 8) & 0xff;
            const b = (this.machine.ppu.palettes[0].RGBColors[i] >> 16) & 0xff;
            // if (i % 4 == 0) {
        ...
        for (let i = 0; i < 16; i++) {
            const r = this.machine.ppu.palettes[1].RGBColors[i] & 0xff;
            const g = (this.machine.ppu.palettes[1].RGBColors[i] >> 8) & 0xff;
            const b = (this.machine.ppu.palettes[1].RGBColors[i] >> 16) & 0xff;
            // if (i % 4 == 0) {
    ...
    public DrawNametableWithOffset(canvasID: string, x: number, y: number, fx: number, fy: number, nameTableIndex: number) {
        ...
        const nameTable = this.machine.ppu.nametables;

        let cntX = x;
        let cntY = y;
        let cntFx = fx;
        let cntFy = fy;
        let cntNameTableIndex = nameTableIndex;
        ...
        for (let scanline = 0; scanline < 240; scanline++) {
            ...
            for (let dot = 0; dot < 256; dot++) {
                ...
                const tile: Tile = this.machine.ppu.tiles[baseTile + nameTable[this.machine.ppu.getRealNametableNo(cntNameTableIndex)].getTileIndex(cntX, cntY)];
                // buffer[scanline << 8 + dot] = tile[(scanline % 8) << 8 + (dot % 8)];
                const dotLow2Bits = tile.pixels[(cntFy << 3) + cntFx];
                // var dotHigh2Bits = nameTable[cntNameTableIndex].getAttrib(cntX, cntY);
                const dotHigh2Bits = (this.machine.ppu.attrTables[this.machine.ppu.getRealNametableNo(cntNameTableIndex)].getAttr(cntX, cntY) << 2);
                // var dotHigh2Bits = 0;
        ...
        const canvasImageData = canvasContext.getImageData(0, 0, 256, 240);
